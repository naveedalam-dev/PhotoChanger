...
const Footer: React.FC<FooterProps> = ({ isOnDressingScreen = false }) => {
  ...
  return (
    <footer className={`fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-md border-t border-gray-200/60 p-3 z-40 ${isOnDressingScreen ? 'hidden md:block' : ''}`}>
        ...
        <div className="h-4 mt-1 sm:mt-0 flex items-center overflow-hidden">
            <AnimatePresence mode="wait">
              <motion.p
                ...
                className="text-center sm:text-right hidden lg:block"
              >
